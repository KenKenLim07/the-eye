...
export default function BiasTrendsPage() {
  ...
  const loadData = async (period: string, direction?: string) => {
    setIsLoading(true);
    try {
      const [summaryData, articlesData] = await Promise.all([
        fetchBiasSummary(Number(period)),
        fetchBiasArticles(direction, 20)
      ]);
  ...
  const handlePeriodChange = async (period: string) => {
    ...
    await loadData(period, selectedDirection);
    setIsPeriodChanging(false);
  };

  const handleRefresh = async () => {
    setIsRefreshing(true);
    await loadData(selectedPeriod, selectedDirection);
    setIsRefreshing(false);
  };

  useEffect(() => {
    loadData(selectedPeriod, selectedDirection);
    ...
    return (
      <div className="container mx-auto p-6">
        <div className="text-center">
          ...
          <Button onClick={() => loadData(selectedPeriod, selectedDirection)}>
            Try Again
          </Button>
        </div>
      </div>
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <Select value={selectedPeriod} onValueChange={handlePeriodChange} disabled={isPeriodChanging}>
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={chartData}>
                ...
                <Tooltip />
                <Legend />
                <Line type="monotone" dataKey="Pro-Government" stroke={COLORS.pro_government} strokeWidth={2} />
                <Line type="monotone" dataKey="Pro-Opposition" stroke={COLORS.pro_opposition} strokeWidth={2} />
