...
\begin{itemize}
\item Automate the large-scale collection and aggregation of English-language news articles from multiple major Philippine online news outlets.
\item Centralize retrieved articles into a unified data storage platform that supports continuous data ingestion and query-based retrieval.
\item Implement text preprocessing techniques optimized for news media content, including tokenization, lemmatization, and noise filtering.
\item Apply hybrid sentiment analysis models combining lexicon-based and neural approaches to enhance polarity classification accuracy.
\item Perform Named Entity Recognition (NER) to identify and extract entities associated with sentiment patterns.Visualize sentiment and entity-level trends through an interactive dashboard for near real- time monitoring and analytical interpretation.
\end{itemize}
\subsection{Conceptual Framework of the Study}
\label{conceptual-framework-of-the-study}
...
\begin{figure*}[t]
  ...
  \label{fig:ipo}
\end{figure*}
Figure~\ref{fig:ipo} illustrates the Input--Process--Output (IPO) conceptual framework of the study. This framework describes how raw news data is collected, processed through analytical methods, and transformed into interpretable outputs for media sentiment monitoring.
...
This study contributes to the intersection of computational linguistics and media monitoring by providing a scalable framework for the automated analysis of Philippine online news. By implementing a hybrid Natural Language Processing (NLP) pipeline that integrates lexicon-based sentiment analysis \cite{ref08} with neural-based entity recognition \cite{ref26}, the system enables continuous and structured monitoring of sentiment patterns across digital news sources. This approach establishes a baseline for high-frequency sentiment observation within the Philippine online news ecosystem.
...
Organizations dedicated to media accountability may use automated monitoring tools to complement existing manual review processes. The system can assist in identifying long-term agenda-setting indicators and shifts in media tone that may warrant further investigation through traditional content analysis methods \cite{ref11}.

\textbf{The General Public and Digital News Consumers.}

Through interactive sentiment visualizations and aggregated news indicators, the system promotes improved media literacy by enabling readers to observe how emotional tone evolves within news reporting. Presenting large volumes of articles in summarized visual form allows users to interpret news discourse more critically rather than relying solely on individual articles \cite{ref34}.
...
The system also serves as a practical implementation reference for hybrid NLP pipelines applied to real-world media datasets. By integrating technologies such as FastAPI \cite{ref27}, Celery \cite{ref29}, and Supabase \cite{ref19}, the study demonstrates how distributed services can support scalable data collection, processing, and visualization in localized analytical environments.
...
A branch of Artificial Intelligence (AI) that enables computers to analyze and interpret human language in textual form \cite{ref01}.

\textbf{Sentiment Analysis.}\\
A Natural Language Processing technique used to computationally identify and categorize the emotional tone expressed in text as positive, neutral, or negative \cite{ref08,ref10}.

\textbf{Polarity.}\\
The directional orientation of sentiment within a text, typically represented as a numerical score or categorical label indicating positive, neutral, or negative sentiment \cite{ref08}.

\textbf{Named Entity Recognition (NER).}\\
A Natural Language Processing method that automatically detects and classifies named entities in text into predefined categories such as persons, organizations, and locations \cite{ref12,ref26}.

\textbf{Web Scraping.}\\
The automated extraction of structured data from websites using browser automation tools or HTTP requests. In this study, it refers to the collection of news articles from selected Philippine news sources for analysis \cite{ref30}.

\textbf{Media Sentiment.}\\
The overall emotional tone conveyed by news content toward specific entities, topics, or events, which may serve as an indicator for exploring patterns in media framing and agenda-setting \cite{ref04,ref11}.

\textbf{Visualization Dashboard.}\\
A graphical user interface that presents processed analytical data through interactive charts, tables, and visual components to support interpretation of sentiment trends \cite{ref31,ref34}.

\textbf{Philippine News Media.}\\
Digital news organizations and online publications operating within the Philippines that serve as the primary sources of textual data analyzed in this study \cite{ref03,ref17}.
...
\subsection{Lexicon- and Rule-Based Approaches}
\label{lexicon--and-rule-based-approaches}

Lexicon- and rule-based approaches remain influential due to their interpretability. Through extensive validation, the VADER framework was shown to reliably capture polarity and intensification, establishing it as a practical baseline for high-volume environments \cite{ref08}.

\subsection{Machine Learning and Transformer Approaches}
\label{machine-learning-and-transformer-approaches}

Subsequent studies expanded sentiment analysis through deep-learning architectures. Transformer-based systems such as BERT and RoBERTa significantly outperform traditional classifiers in detecting nuanced sentiment but present challenges regarding computational cost and domain-specific shifts \cite{ref10,ref14}.

\subsection{Hybrid Approaches}
\label{hybrid-approaches}

Combining lexicon-based features with supervised machine-learning improves robustness in noisy and informal text domains \cite{ref09}.

\subsection{Targeted / Entity-Level Sentiment Analysis}
\label{targeted-entity-level-sentiment-analysis}

Recent work emphasizes targeted sentiment analysis, associating polarity with specific actors or institutions rather than assigning a single score to a document \cite{ref12}.

Recent syntheses by Leon \cite{ref10} and Rana \cite{ref14} clarify the evolution from rule-based heuristics to neural architectures. Leon \cite{ref10} argues that while the shift to Transformers represents a paradigm change in accuracy, systems must reconcile the transparency of earlier lexicon models with the predictive power of modern architectures. Similarly, Rana \cite{ref14} highlights that Transformer-based models are superior in capturing nuanced emotional cues and sarcasm that traditional lexicons often misinterpret. These reviews justify the multi-model approach of the present study, balancing high-speed lexicon processing with the deep contextual understanding of modern pipelines.
...
\subsection{Sentiment of Newspaper Headlines}
\label{sentiment-of-newspaper-headlines}

Diaz \cite{ref03} examined headlines from major Philippine broadsheets, finding that most exhibited negative sentiment, suggesting emotionally charged framing aimed at capturing attention. While insightful, the manual sampling design and focus on headlines rather than full articles prevented continuous analysis.

\subsection{COVID-19 Sentiment in Metro Manila}
\label{covid-19-sentiment-in-metro-manila}

Complementing news-centric studies, Garcia \cite{ref05} analyzed the emotional landscape of Metro Manila during the COVID-19 pandemic through social media. This study demonstrated how computational tools could track public anxiety in response to policy shifts, though it highlighted the difficulty of analyzing Philippine-specific contexts using generic tools.

\subsection{Fake News and Economic Monitoring}
\label{fake-news-and-economic-monitoring}

Other local research investigated fake news classification, finding that sentiment features can assist in distinguishing stylistic differences across sources \cite{ref23}. Furthermore, the Bangko Sentral ng Pilipinas developed high-frequency News Sentiment Indices that correlated strongly with economic indicators, demonstrating the value of media sentiment as a macroeconomic signal \cite{ref22}.

\subsection{Institutional and Structural Monitoring}
\label{institutional-and-structural-monitoring}

Studies by the Philippine Institute for Development Studies \cite{ref25} revealed divergent sentiment patterns toward government policies, while the Center for Media Freedom and Responsibility \cite{ref17} provided insights into issue salience during election cycles through human coding. Finally, the Media Ownership Monitor Philippines \cite{ref24} investigates structural media conditions but is updated only at multi-year intervals.

\subsection{Philippine NLP Baselines}
\label{philippine-nlp-baselines}

A technical foundation for localized automation is provided by Cruz and Cheng \cite{ref02}, who established baseline performance metrics for text classification in Filipino. Their work demonstrated that fine-tuning pre-trained Transformer models on local datasets significantly outperforms traditional machine learning for Philippine text, providing the justification for utilizing advanced architectures to handle unique syntax and code-switching in local news.

\subsection{Methods-in-the-Literature}
\label{methods-in-the-literature}
...
\begin{itemize}
\item \textbf{Lexicon / Rule-based (e.g., VADER)}: Social media, news headlines (Advantages: Fast, interpretable, low latency.) (Limitations: Domain mismatch, sarcasm \cite{ref08}.)
\item \textbf{Transformer models (BERT)}: High-accuracy tasks (Advantages: Strong contextual understanding.) (Limitations: High compute cost, domain-shift \cite{ref10,ref14}.)
\item \textbf{Hybrid Pipelines}: Noisy domains, limited data (Advantages: Production-ready, robust.) (Limitations: Pipeline complexity \cite{ref09}.)
\item \textbf{NER Tools (spaCy)}: Entity-level mapping (Advantages: Balances performance/interpretability.) (Limitations: Requires domain adaptation \cite{ref12,ref26}.)
\end{itemize}
...
\section{Project Technicality}

\section{Project Technicality}
\label{technicality-of-the-project}
...
The project also employs a hybrid computational linguistics framework that integrates lexicon-based sentiment analysis with neural Named Entity Recognition. Lexicon-based sentiment scoring, implemented through the VADER model, provides interpretable polarity scores for textual content by evaluating the presence and intensity of sentiment-bearing words \cite{ref08}. At the same time, Named Entity Recognition models are used to identify important entities such as persons, organizations, and locations within news articles, providing additional contextual information about the content being analyzed \cite{ref26}. The combination of these approaches allows the system to generate richer analytical outputs that include both emotional polarity and structural entity information.
...
\subsection{Details of the Technologies to be Used}
\label{details-of-the-technologies-to-be-used}
...
The project uses Playwright \cite{ref30}, a headless browser automation framework, to extract data from JavaScript-driven news platforms. Playwright enables full browser rendering before content extraction, ensuring accurate retrieval of complete articles.

\textbf{Backend Service Architecture}

The backend is implemented using FastAPI \cite{ref27}, a high-performance Python web framework optimized for asynchronous operations. The backend follows RESTful architectural principles, using Uvicorn \cite{ref28} as the ASGI server. This design cleanly separates internal data processing services from the user-facing frontend.

\textbf{Distributed Task Processing}

To prevent computational bottlenecks, the system employs Celery \cite{ref29} as a distributed task queue, with Redis \cite{ref15} acting as the message broker and result backend. Celery enables asynchronous execution of scraping and NLP-related tasks independent of the main API process. Redis functions as a lightweight, in-memory data store that manages task queues and intermediate results efficiently \cite{ref15}.
...
\def\labelenumi{\arabic{enumi}.}
\item
  Lexicon-based sentiment analysis: Utilizing the VADER framework \cite{ref08} for computing polarity scores over article text. VADER is specifically tuned for sentiment expression in social and news media \cite{ref08}.
\item
  spaCy \cite{ref26}: Used for Named Entity Recognition (NER), specifically identifying persons, organizations, and locations within news articles.
\end{enumerate}

\textbf{Data Storage and Persistence}

The system uses PostgreSQL \cite{ref06}, provisioned through Supabase \cite{ref19}, as its primary relational database. PostgreSQL ensures reliability and strong ACID guarantees for article metadata and analytical results \cite{ref06}. Additionally, the analysis pipeline utilizes NumPy \cite{ref07}, SciPy \cite{ref20}, Scikit-learn \cite{ref13}, and pandas \cite{ref36} for structured data manipulation and fundamental algorithms \cite{ref20,ref36}.

\textbf{Frontend Analytical Interface}

The frontend is developed using Next.js \cite{ref31}, React \cite{ref32}, and TypeScript, providing a responsive analytics dashboard. For visualization, the system uses Recharts \cite{ref34} to render sentiment timelines and source comparisons. Tailwind CSS \cite{ref33} is used for utility-first styling
...
\begin{itemize}
\item \textbf{Scalability}: Celery \cite{ref29}, Redis \cite{ref15}, FastAPI \cite{ref27} (Advantages: Handles concurrent dashboard requests and async background tasks..)
\item \textbf{Modularity}: Supabase \cite{ref19}, Next.js \cite{ref31} (Advantages: Clear separation of persistence, API, and presentation layers..)
\item \textbf{Performance}: Playwright \cite{ref30}, Redis \cite{ref15} (Advantages: Enables non-blocking operations and efficient data caching..)
\item \textbf{Reliability}: PostgreSQL \cite{ref06}, Docker \cite{ref35} (Advantages: Ensures data integrity and consistent deployment environments \cite{ref35}..)
\end{itemize}
...
\begin{figure*}[t]
  ...
  \label{fig:sdlc}
\end{figure*}
The implemented system follows an \textbf{iterative Software Development Life Cycle (SDLC)}, as depicted in Figure~\ref{fig:sdlc} This model was chosen because the system involves continuously changing online news sources, requiring frequent adaptation and refinement of the scraping, natural language processing, and dashboard modules.
...
Automated Web Scraping: Automatically extract articles from six (6) Philippine news outlets (GMA, Rappler, Inquirer, Philstar, Manila Bulletin, and Manila Times) using Playwright \cite{ref30}at scheduled intervals \cite{ref15,ref29}.

Data Normalization: Clean extracted text using BeautifulSoup4 and NLTK \cite{ref39,ref01} to remove HTML and whitespace.

Duplicate Detection: Check for existing URLs to prevent redundant processing.

Sentiment Polarity Scoring: Utilize the VADER \cite{ref08} tool to assign positive, negative, and neutral scores.

Named Entity Recognition (NER): Utilize spaCy \cite{ref26} to identify Persons (PER), Organizations (ORG), and Locations (LOC).

Data Visualization: Provide a dashboard for sentiment trends and entity frequencies using Tailwind CSS and Recharts \cite{ref34,ref33}.
...
\item
  Scalability: Backend architecture (FastAPI \cite{ref27} and Celery \cite{ref29}) must support adding news sources without significant code changes.
\item
  Reliability: Use Redis \cite{ref15} as a message broker to ensure task persistence.
\item
  Usability: The web dashboard must be intuitive, with a responsive design using Next.js,Tailwind and Recharts \cite{ref32,ref33,ref34} that allows users to view data on both desktop and mobile devices.
\item
  Data Integrity: Ensure foreign key constraints in the PostgreSQL \cite{ref06} database.
\end{enumerate}
...
\subsection{Output and User-Interface Design}
\label{output-and-userinterface-design}
...
\section{News Dashboard (Home Screen)}
\label{news-dashboard-home-screen}
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{ui-homepage.png}
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{inquirer-sentiment-distribution.png}
  \caption{Sentiment distribution of Inquirer articles.}
  \label{fig:inquirer-sentiment-distribution}
\end{figure*}
\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{rappler-sentiment-distribution.png}
  \caption{Sentiment distribution of Rappler articles.}
  \label{fig:rappler-sentiment-distribution}
\end{figure*}
\subsection{Source-Level Sentiment Comparison (Rappler vs. Inquirer)}
\label{source-level-sentiment-comparison-rappler-vs.-inquirer}

Figures~\ref{fig:inquirer-sentiment-distribution} and~\ref{fig:rappler-sentiment-distribution} presents a source-specific sentiment comparison between \textbf{Rappler} and \textbf{Inquirer} over the same seven-day observation window. By applying the source filter in the Sentiment Trends module, the system isolates articles from individual news outlets and computes sentiment distributions independently.
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{gma-timeline.png}
  \caption{Sentiment data and timeline (GMA).}
  \label{fig:gma-timeline}
\end{figure*}
\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{manila-times-timeline.png}
  \caption{Sentiment data and timeline (Manila Times).}
  \label{fig:manila-times-timeline}
\end{figure*}
\textbf{GMA Average Sentiment (see Figure~\ref{fig:gma-timeline})}
...
\item March 6: 0.042
\end{itemize}
\textbf{Manila Times Average sentiment (see Figure~\ref{fig:manila-times-timeline})}
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{search-crime.png}
  \caption{Search results example for the keyword ``crime''.}
  \label{fig:search-crime}
\end{figure*}
As shown in Figure~\ref{fig:search-crime}, the system provides a query feature that allows users to filter the historical database by keywords. In this example, a search for the keyword \emph{\textbf{``crime''}} was performed across the collected news corpus. The retrieved articles predominantly exhibit Negative sentiment, reflecting the lexicon-based classification patterns of the VADER analyzer for crime-related news, which is consistent with the generally negative nature of crime-related news reporting.
...
\section{Article Quick View (Modal Dialog)}
\label{article-quick-view-modal-dialog}
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{quick-view-modal.png}
...
This report presents a ranked list of extracted entities (Persons, Organizations, and GPEs) \cite{ref26} paired with their mention frequency and mean sentiment values. Integrated into the correlation module, it allows users to determine the prevailing emotional tone directed toward specific actors within the same analytical timeframe, facilitating a deeper understanding of entity portrayal in the news.
...
The system employs a relational data model implemented in PostgreSQL via Supabase \cite{ref06,ref19}. The design organizes the collected news data, NLP outputs, and operational logs to ensure traceability, consistency, and analytical flexibilit\textbf{y}. Core logical entities, their attributes, and relationships are described below.

\textbf{Core Entities}

\begin{itemize}
\item \textbf{Source}: Source\_name (Advantages: Represents each news outlet. Serves as the primary reference for articles and scraping logs. One source may publish multiple articles and have multiple associated scraping runs..)
\item \textbf{Article}: Id, source\_name, title, url, published\_at, category (Advantages: Stores metadata and content of each scraped news article. The id ensures uniqueness, while source\_name links to the corresponding Source. Each Article may have multiple NLP analysis records over time..)
\item \textbf{Analysis}: Id, article\_id, model\_type, model\_version, sentiment\_score, sentiment\_label, created\_at (Advantages: Stores NLP outputs for each article, including sentiment score, sentiment classification, model metadata, and processing timestamps. Multiple entries may exist per article to accommodate repeated or updated analyses..)
\item \textbf{Scraping log}: Id, source\_name, status, articles\_scraped, started\_at, completed\_at (Advantages: Records operational telemetry for each scraping run, including execution status, number of articles collected, and timing. Enables auditing of the data collection process..)
\item \textbf{Entity Statistics}: entity\_name, entity\_type, mention\_frequency, average\_sentiment (Advantages: Aggregated information for Persons, Organizations, and Geopolitical Entities. Generated dynamically from Article and Analysis tables via analytical queries; it is \textbf{not persistently stored} in the database..)
\end{itemize}
...
\begin{itemize}
\item \textbf{Operating System (Host)}: Windows 10/11, MacOS Sonoma (Advantages: Local development environment.) (Limitations: ---.)
\item \textbf{Python (Host)}: 3.14.2 (Advantages: Local scripting/testing environment.) (Limitations: \cite{ref37}.)
\item \textbf{Node.js}: v24.13.0 (Advantages: Frontend runtime/tooling.) (Limitations: \cite{ref38}.)
\item \textbf{npm}: 11.6.2 (Advantages: Package management/build scripts.) (Limitations: \cite{ref40}.)
\item \textbf{Frontend Framework}: Next.js 15.5.2 (Advantages: Web UI and routing.) (Limitations: \cite{ref31}.)
\item \textbf{UI Runtime}: React 19.1.0 + React DOM (Advantages: Component-based frontend rendering.) (Limitations: \cite{ref32}.)
\item \textbf{Styling}: Tailwind CSS 4 (Advantages: Utility-first frontend styling.) (Limitations: \cite{ref33}.)
\item \textbf{Charts}: Recharts 3.2.0 (Advantages: Trends/correlation visualizations.) (Limitations: \cite{ref34}.)
\item \textbf{Frontend Data Client}: @supabase/supabase-js 2.56.1 (Advantages: Supabase access from frontend.) (Limitations: \cite{ref19}.)
\item \textbf{Backend Framework}: FastAPI 0.104.1 (Advantages: REST API endpoints.) (Limitations: \cite{ref27}.)
\item \textbf{ASGI Server}: Uvicorn 0.30.1 (Advantages: Backend app serving.) (Limitations: \cite{ref28}.)
\item \textbf{Task Queue/ Scheduler}: Celery Beat + celery-redbeat (Advantages: Tasker/Async scraping/analysis jobs.) (Limitations: \cite{ref29}.)
\item \textbf{Queue/Cache}: Redis 7 (image), redis-py 5.0.4 (Advantages: Broker + caching.) (Limitations: \cite{ref15}.)
\item \textbf{Database}: Supabase (PostgreSQL) (Advantages: Persistent relational storage.) (Limitations: \cite{ref06,ref19}.)
\item \textbf{Python Supabase Client}: supabase 2.6.0 (Advantages: Backend DB access.) (Limitations: \cite{ref19}.)
\item \textbf{Scraping HTTP}: httpx 0.27.0, requests 2.31.0 (Advantages: Network fetches.) (Limitations: \cite{ref41}.)
\item \textbf{HTML Parsing}: beautifulsoup4 4.12.3, lxml (Advantages: Article parsing/cleanup.) (Limitations: \cite{ref39}.)
\item \textbf{Browser Automation}: Playwright 1.46.0 (Advantages: Dynamic/anti-bot scraping.) (Limitations: \cite{ref30}.)
\item \textbf{Sentiment NLP}: NLTK 3.8.1 (VADER) (Advantages: Sentiment scoring.) (Limitations: \cite{ref01,ref08}.)
\item \textbf{NER NLP}: spaCy 3.7.4 (Advantages: Entity extraction.) (Limitations: \cite{ref26}.)
\item \textbf{Stats/Math}: NumPy, SciPy, Pandas, scikit-learn (Advantages: Correlation/statistical analytics.) (Limitations: \cite{ref07,ref13}, \cite{ref20}, \cite{ref36}.)
\item \textbf{Containerization}: Docker + Docker Compose (Advantages: Service orchestration.) (Limitations: \cite{ref35}.)
\end{itemize}
...
The front-end interface of the system was developed using \textbf{Next.js \cite{ref31}, a React-based web framework that enables efficient component-based interface development and server-side rendering capabilities. The application was implemented using \textbf{TypeScript} to improve type safety and maintain code reliability.

For styling and layout, \textbf{Tailwind CSS \cite{ref33} was used to provide a responsive and utility-first design framework. UI components were implemented using the \textbf{shadcn/ui component library}, which provides reusable and accessible interface elements built on top of \textbf{React \cite{ref32} and Radix UI primitives.

Data visualization components used in the dashboard---including sentiment trend charts and correlation visualizations---were implemented using the \textbf{Recharts charting library \cite{ref34}, which integrates seamlessly with React applications and enables dynamic rendering of statistical data.

\textbf{Back End}

The back-end services of the system were implemented using \textbf{FastAPI \cite{ref27}, a Python-based web framework designed for high-performance API development. FastAPI provides asynchronous request handling and efficient data serialization, enabling the system to support real-time analytics queries from the dashboard interface.

Background processing tasks such as automated scraping and sentiment analysis were executed using \textbf{Celery \cite{ref29}, a distributed task queue system. Celery Beat was used to schedule periodic scraping jobs that continuously collect news articles from supported sources.

\textbf{Redis \cite{ref15} served as the task broker and caching layer for Celery workers, ensuring efficient job distribution and improved response performance.

Persistent data storage was handled by \textbf{Supabase \cite{ref19}, which provides a managed \textbf{PostgreSQL \cite{ref06} database environment.

To ensure consistent deployment and environment isolation, the application services were containerized using \textbf{Docker \cite{ref35}, with Docker Compose used to orchestrate the API server, background workers, and supporting services during development and testing.
...
The \textbf{data acquisition layer} is responsible for collecting news articles from supported Philippine news sources through automated scraping routines. Scraper jobs are executed asynchronously using scheduled background tasks, ensuring that article collection occurs regularly without interrupting the main application services. The scraping mechanism utilizes browser automation through Playwright \cite{ref30} to extract article content and metadata from dynamically rendered news pages.

The \textbf{processing layer} performs natural language processing operations on the collected articles. This includes sentiment classification using the VADER sentiment analyzer \cite{ref08} and entity extraction using spaCy's Named Entity Recognition (NER) pipeline \cite{ref26}. The processing tasks are executed by distributed worker processes managed by Celery \cite{ref29}, enabling parallel handling of article analysis workloads. The task queue is coordinated through Redis \cite{ref15}, which functions as the message broker for distributing background processing jobs.

The \textbf{data storage layer} utilizes a PostgreSQL relational database \cite{ref06} hosted on Supabase \cite{ref19}. This layer stores the scraped article records, sentiment analysis outputs, and operational scraping logs. The relational schema supports structured querying and enables efficient generation of analytical reports such as sentiment timelines, entity frequency distributions, and cross-source sentiment correlation matrices.

The \textbf{presentation layer} is implemented as a web application built using Next.js \cite{ref31} and React \cite{ref32}, with styling provided through Tailwind CSS \cite{ref33}. Data visualization components, including sentiment trend charts and statistical dashboards, are rendered using the Recharts visualization library \cite{ref34}. This interface provides interactive dashboards that allow users to explore sentiment trends, compare media sources, and analyze entity-level sentiment patterns within the collected news dataset.
